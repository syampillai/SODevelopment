...
public class DefineAssembly<T extends InventoryItemType, C extends InventoryItemType> extends DataTreeGrid<InventoryAssembly> implements Transactional {
    ...
    public DefineAssembly(String itemTypeClass) {
        this(createClass(itemTypeClass, false), createClass(itemTypeClass, true));
    }

    private static <O extends InventoryItemType> Class<O> createClass(String itemTypeClass, boolean component) {
        int p = itemTypeClass.indexOf('|');
        if(component) {
            if(p < 0) {
                return null;
            }
            return createClass(itemTypeClass.substring(p + 1), false);
        }
        if(p > 0) {
            itemTypeClass = itemTypeClass.substring(0, p);
        }
        try {
            //noinspection unchecked
            return (Class<O>) JavaClassLoader.getLogic(itemTypeClass);
        } catch (Throwable e) {
            throw new SORuntimeException("Invalid Item Type Class: " + itemTypeClass);
        }
    }
