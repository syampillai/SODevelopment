...
public class FileField extends AbstractObjectField<StreamData> {
    ...
    public FileField(String label, ObjectField.Type... types) {
        ...
        if(containsAny(ObjectField.Type.FILE, ObjectField.Type.IMAGE, ObjectField.Type.AUDIO, ObjectField.Type.VIDEO)) {
            upload = new ImageButton("Upload file", VaadinIcon.UPLOAD, clicked).withBox();
            link = new ImageButton("Link to ...", VaadinIcon.LINK, clicked).withBox();
            link.setVisible(false);
        }
        if(contains(ObjectField.Type.STILL_CAMERA)) {
            captureImage = new ImageButton("Take photo", VaadinIcon.CAMERA, clicked).withBox();
        }
        if(contains(ObjectField.Type.VIDEO_CAMERA)) {
            captureVideo = new ImageButton("Shoot video", VaadinIcon.MOVIE, clicked).withBox();
        }
        if(contains(ObjectField.Type.MIC)) {
            captureAudio = new ImageButton("Record audio", VaadinIcon.MICROPHONE, clicked).withBox();
        }
        remove = new ImageButton("Remove", VaadinIcon.CLOSE, clicked).withBox();
        remove.setVisible(false);
        uploadCancel = new ImageButton("Cancel", VaadinIcon.CLOSE, clicked).withBox();
        ...
        view = new ImageButton("View", VaadinIcon.EYE, clicked).withBox();
        download = new ImageButton("Download", VaadinIcon.DOWNLOAD, clicked).withBox();
        setLabel(label);
    ...
    private class ImageForm extends AbstractMediaForm {
        ...
        protected void buildButtons() {
            super.buildButtons();
            shoot = new ImageButton("Shoot", VaadinIcon.CAMERA,this).withBox();
            preview = new ImageButton("Show video", VaadinIcon.MOVIE,this).withBox();
    ...
    private abstract class MediaForm extends AbstractMediaForm {
        ...
        protected void buildButtons() {
            super.buildButtons();
            view = createViewButton().withBox();
            startRecording = new ImageButton("Record", createRecordIcon(),this).withBox();
            stopRecording = new ImageButton("Stop recording", VaadinIcon.STOP,this);
            stopRecording.setStyle("color", "red");
            stopRecording.withBox();
