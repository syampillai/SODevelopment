...
public class ManageApplication extends DataForm implements Transactional {
    ...
    protected boolean process() {
        ...
        while(true) {
            if(!a.isEmpty()) {
                if(StringUtility.isLetterOrDigit(a)) {
                    break;
                }
                if(a.endsWith("-scheduler") || a.endsWith("-connector")) {
                    if(StringUtility.isLetterOrDigit(a.substring(0, a.indexOf('-')))) {
                        break;
                    }
                }
            }
        ...
        if(a.equals(getApplication().getLinkName()) && action.getIndex() == 1 && !warned) {
            warning.clearContent().append("You are about to stop this application! Press the 'Proceed' button again.", "red").update();
            warned = true;
            return false;
        }
        try {
            String manager = ApplicationServer.getGlobalProperty("application.manager", "https://localhost:8443/manager");
