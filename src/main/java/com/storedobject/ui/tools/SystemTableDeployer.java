...
public class SystemTableDeployer extends View implements Transactional {
    ...
    public SystemTableDeployer() {
        super("System Data Class Deployer");
        ...
        upload = new PopupButton("Upload File", VaadinIcon.UPLOAD);
        ((PopupButton)upload).add(uploadProcess, uploadProcessAndReindex);
        buttons.add(upload);
    ...
    private void setKlass(String currentClassName) throws Exception {
        ...
        if(JavaClassLoader.loaded(currentClassName) || !StoredObject.class.isAssignableFrom(objectClass) || objectClass == StoredObject.class) {
            throw new Exception("Not a System Class: " + currentClassName + " (If it is really a System Class, try after restartung the application)");
        }
        ca = StoredObjectUtility.classAttribute(objectClass);
        if(ca == null) {
            throw new Exception("Invalid System Class: " + currentClassName);
        }
    }
    ...
    public void clicked(Component c) {
        ...
        if(c == uploadProcess || c == uploadProcessAndReindex) {
            String p = password();
            if(p != null) {
    ...
    private static ArrayList<String> checkAlterTable(ClassAttribute<?> ca) throws Exception {
        ...
        String tableName = ca.getModuleName() + "." + ca.getTableName(),
                pTableName = pca.getModuleName() + "." + pca.getTableName();
        String tableNameH = tableName.replace(".", ".H_"), pTableNameH = pTableName.replace(".", ".H_");
    ...
    private class MultipleClasses extends UploadProcessorView {
        ...
        public MultipleClasses(String password, boolean reindex) {
            super((reindex ? "Re-index" : "Process") + " Multiple Classes", "File containing list of classes");
        ...
        private void process(InputStream data) throws Exception {
            m((reindex ? "Re-index" : "Creating/updating/index") + "ing...");
            boolean error;
            BufferedReader br = new BufferedReader(new InputStreamReader(data, StandardCharsets.UTF_8));
            String klass;
            while((klass = br.readLine()) != null) {
                klass = klass.trim();
                if(klass.isEmpty() || !klass.startsWith("com.storedobject.") || klass.equals("com.storedobject.core.StoredObject")) {
                    continue;
                }
                p(klass);
                try {
