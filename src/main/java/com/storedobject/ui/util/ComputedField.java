...
public class ComputedField<T extends ComputedValue<P>, P> extends CustomField<T> {
    ...
    private Checkbox check = new Checkbox(true);
        ...
        check.addClickListener((e) -> {
            if (e.isFromClient()) {
                ...
                try {
                    ...
                    v = (T)this.getValue().clone();
                } catch (CloneNotSupportedException ignored) {
                    return;
                }
                v.reverseStatus();
                this.setModelValue(v, true);
                getField().setReadOnly(!this.check.getValue());
    ...
    protected T generateModelValue() {
        T value = this.getEmptyValue();
        value.setValue(getField().getValue());
